...
// Captain accepts ride - REDIS ONLY VERSION
socket.on("accept-ride", async ({ rideId, captainId }) => {
    ...
    try {
      ...
      // Get ride data from Redis to find riderId
      ...
      // 🔧 FIXED: Add missing 'id' field to Redis data
      const completeRideData = {
        ...rideData,
        id: rideId // Add the missing id field that rider app expects
      };

      console.log("🔍 DEBUG RIDER NOTIFICATION:", completeRideData);
      console.log("  - Looking for riderId:", riderId, "type:", typeof riderId);
      console.log("  - String version:", String(riderId));
      console.log("  - riders.get(String(riderId)):", riders.get(String(riderId)));
      console.log("  - All rider keys:", Array.from(riders.keys()));
      console.log("  - Full riders map:", Object.fromEntries(riders));
      console.log("  - Complete ride data with id:", completeRideData);
      
      const riderSocket = riders.get(String(riderId));
      if (riderSocket) {
        io.to(riderSocket).emit("ride-assigned", completeRideData);
        console.log(`✅ Notified rider ${riderId} about ride assignment with complete Redis data`);
      } else {
